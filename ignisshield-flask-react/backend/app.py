...
def ensure_data_dir():
    ...
    if not os.path.isfile(ALERTS_FILE):
        with open(ALERTS_FILE, 'w', encoding='utf-8') as f:
            json.dump([], f)
    if not os.path.isfile(HOTSPOTS_FILE):
...
def root():
    return jsonify({"ok": True, "service": "IgnisShield Flask API"})
...
def alert():
    ...
    return jsonify({"ok": True, "delivered_count": len(deliveries), "delivered": deliveries})
...
@app.route('/realtime/firms', methods=['POST'])
def realtime_firms():
    ensure_data_dir()
    p = request.get_json(force=True)
    ...
    if triggered:
        ...
        alerts_sent = True
    return jsonify({"ok": True, "hotspots": hotspots, "alerts_sent": alerts_sent, "triggered_hotspots": triggered})
